...
func main() {
	// 対象URLおよび取得する記事数を指定
	target_url := "https://russian.rt.com/"
	num_articles := 10

	// 開始メッセージをSlackに投稿
	sendtoSlack("Russia Todayによる最新のニュースを取得します。\n")
	...
	// 各記事の情報を順に取得
	doc.Find("a.link.link_color ").Each(func(i int, s *goquery.Selection) {
		...
		href, _ := s.Attr("href")
		article_url := "https://russian.rt.com" + href
